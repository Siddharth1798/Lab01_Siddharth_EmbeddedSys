...
void PI_update(void) {
    ...
    // Motor Speed in encoder counts and we need to convert to RPM
    //motor_speed = (motor_speed / 2);
    error = target_rpm - (motor_speed/2); // Calculate error signal
    ...
    int16_t output = (Ki * error_integral) + (Kp * error); // Changed to calculate kp, ki, error and error integral to get ouput coefficient.
     ...
     */
     ...
     /// TODO: Divide the output into the proper range for output adjustment
       output = output >> 5; // Divide by 32 because we chose 3200
